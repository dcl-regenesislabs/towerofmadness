...
export type PointLeaderboardPanelEntry = {
  name: string
  points: string
}

export type PointLeaderboardPanelOptions = {
  parent?: Entity
...
const DEFAULT_TABS = ['HISTORIC POINTS', 'WEEKLY POINTS']
const DEFAULT_ROWS = 10
const DEFAULT_TAB_DATA: PointLeaderboardPanelEntry[][] = [[], []]

export function createPointLeaderboardPanel(options: PointLeaderboardPanelOptions = {}) {
  ...
  for (let i = 0; i < tabs.length; i++) {
    ...
    const headerPoints = engine.addEntity()
    Transform.createOrReplace(headerPoints, {
      parent: headerRow,
    ...
    VisibilityComponent.createOrReplace(headerPoints, { visible: i === 0 })

    const rows: Entity[] = []
    const visibilityList: Entity[] = [headerName, headerPoints]
    ...
    for (let r = 0; r < DEFAULT_ROWS; r++) {
      ...
      const pointsText = engine.addEntity()
      Transform.createOrReplace(pointsText, {
        parent: rowRoot,
        position: Vector3.create(1.25, 0, 0),
        rotation: Quaternion.Identity(),
        scale: Vector3.One()
      })
      TextShape.createOrReplace(pointsText, {
        text: '--',
        fontSize: 1.5,
        textColor: Color4.White(),
        outlineColor: Color4.Black(),
      ...
      VisibilityComponent.createOrReplace(pointsText, { visible: i === 0 })

      rows.push(nameText)
      rows.push(pointsText)
      visibilityList.push(rankText, nameText, pointsText)
    }
...
export function setActivePointTab(panel: PanelState, index: number) {
...
export function setPointTabData(panel: PanelState, tabIndex: number, entries: PointLeaderboardPanelEntry[]) {
  ...
  if (entries.length === 0) {
    for (let i = 0; i < DEFAULT_ROWS; i++) {
      ...
      const pointsEntity = rowEntities[i * 2 + 1]
      TextShape.getMutable(nameEntity).text = i === 0 ? 'No data' : ''
      TextShape.getMutable(pointsEntity).text = ''
    }
    return
  }
  ...
  for (let i = 0; i < DEFAULT_ROWS; i++) {
    ...
    TextShape.getMutable(pointsEntity).text = entry ? entry.points : '--'
  }
}
